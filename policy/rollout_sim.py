...
the 4 ms control tick to produce qref/qdref, driven through the same
PD + RNEA-feedforward controller used for dataset generation, receding
horizon: the first --exec_steps x 0.1 s are executed, then re-infer.
...
def main():
    ...
    CTRL_DT, NSUB = demo["CTRL_DT"], demo["NSUB"]
    ticks_per_step = int(round(0.1 / CTRL_DT))               # 25

    spec = pol.spec
    dense_s = np.arange(0, a.exec_steps * 0.1, CTRL_DT)
    # spline s=0 is t+0.1: prepend the current-step segment by evaluating
    # from s=0; execution covers absolute t+0.1 .. t+0.1+exec window
